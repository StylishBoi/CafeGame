    ...
    void Update()
    {
        ...
        if (Input.anyKey && buttonCoolDown > 0.5f)
        {
            if (currentPress == arrowOrderInts[currentPosition])
            {
                ...
                Debug.Log("Correct");
                buttonCoolDown = 0;
                _listOfArrowButtons[currentPosition-1].Clicked();
                
                if (repeat)
                {
                    Debug.Log("Shaking left");
                    milkshake.transform.localPosition += new Vector3(0, 100f, 0f)*Time.deltaTime;
                    repeat = false;
                }
                else if (!repeat)
                {
                    Debug.Log("Shaking right");
                    milkshake.transform.localPosition += new Vector3(0, -100f, 0f)*Time.deltaTime;
                    repeat = true;
                }
            }
